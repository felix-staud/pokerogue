      ...
      pokemonConfigs: [
        {
          ...
          mysteryEncounterBattleEffects: (pokemon: Pokemon) => {
            pokemon.scene.unshiftPhase(
              new StatStageChangePhase(pokemon.scene, pokemon.getBattlerIndex(), true, [Stat.SPDEF, Stat.SPD], 1),
            );
          },
        },
        {
  ...
  .withOption(
    MysteryEncounterOptionBuilder.newOptionWithMode(MysteryEncounterOptionMode.DISABLED_OR_SPECIAL)
      ...
      .withOptionPhase(async (scene: BattleScene) => {
        ...
        setEncounterRewards(scene, { fillRemaining: true }, undefined, () => {
...
function giveLeadPokemonAttackTypeBoostItem(scene: BattleScene) {
  ...
  if (leadPokemon) {
    ...
    let boosterModifierType = generateModifierType(
      scene,
      modifierTypes.ATTACK_TYPE_BOOSTER,
    ) as AttackTypeBoosterModifierType;
    if (!boosterModifierType) {
      boosterModifierType = generateModifierType(scene, modifierTypes.ATTACK_TYPE_BOOSTER, [
        Type.FIRE,
      ]) as AttackTypeBoosterModifierType;
    }
